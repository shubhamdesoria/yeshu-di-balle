...
const jesusArray = (count: number) => Array.from({ length: count }, (_, i) => i);
  ...
  return (
    <div className="">
      <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'sticky', top: '0', width: 'fit-content', margin: 'auto', padding: '0px 100px', background: 'red' }}>
        <h2> Yashu the Janamdin</h2>
        <br />
        <div className="buttons">
          <button style={{color: 'green', backgroundColor: 'white', padding: '0.25rem 0.5rem'}} onClick={() => setcount((prevCount) => prevCount + 1)}>+</button>
          <span style={{margin: '0.5rem'}}>{count}</span>
          <button style={{color: 'red', backgroundColor: 'white', padding: '0.25rem 0.5rem'}} onClick={() => {
            setcount((prevCount) => (prevCount !== 0 ? prevCount - 1 : prevCount));
          }}>-</button>
          <button style={{background: 'white', color: 'red', padding: '0.25rem 0.5rem'}} onClick={() => setcount(0)}>RESET</button>
        </div>
      </div>
      <main className="" style={{display: 'flex', flexWrap: 'wrap'}}>
        {count === 0 ? (
          ...
          jesusArray(count).map((value, index) => {
            return (
              <div key={`item-${value}`}>
                <img
                  className=""
                  ...
                  alt="Yeshu"
                />
                Mera Yashu Yashu Count: {index + 1}
              </div>
            )
          })
        )}
